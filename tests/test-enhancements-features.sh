#!/usr/bin/env bash
# AI Memory Kit — Test Suite for Feature Enhancements (#11, #12, #13)
# Tests:
...
mkdir -p "$MOCK_HOME/.claude/memory-kit"
...
# Copy doctor.sh into mock memory-kit
cp "$KIT_DIR/doctor.sh" "$MOCK_HOME/.claude/memory-kit/doctor.sh"
chmod +x "$MOCK_HOME/.claude/memory-kit/doctor.sh"

echo ""
echo "AI Memory Kit — Feature Enhancement Tests"
echo "═════════════════════════════════════════"
...
DOCTOR_OUTPUT="$(HOME="$MOCK_HOME" bash "$MOCK_HOME/.claude/memory-kit/doctor.sh" 2>&1)" && DOCTOR_EXIT=0 || DOCTOR_EXIT=$?
...
mkdir -p "$UNINSTALL_HOME/.claude/memory-kit"
touch "$UNINSTALL_HOME/.claude/memory-kit/doctor.sh"
...
# Verify memory-kit directory was removed
if [ ! -d "$UNINSTALL_HOME/.claude/memory-kit" ]; then
    pass "uninstall.sh removed memory-kit directory"
else
    fail "uninstall.sh did not remove memory-kit directory"
fi
