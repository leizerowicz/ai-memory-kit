...
Look for `~/.claude/memory-kit/journal-summary.sh`. If found, run it based on what the user asked:

```bash
# This week (default)
bash ~/.claude/memory-kit/journal-summary.sh

# Last week
bash ~/.claude/memory-kit/journal-summary.sh --last-week

# Specific date range
bash ~/.claude/memory-kit/journal-summary.sh --from 2026-02-01 --to 2026-02-28

# All history as Markdown (good for Notion/GitHub)
bash ~/.claude/memory-kit/journal-summary.sh --all --format markdown

# Team aggregate (each subdir is a contributor's journal dir)
bash ~/.claude/memory-kit/journal-summary.sh --team ~/team-journals

# Custom journal directory
bash ~/.claude/memory-kit/journal-summary.sh --dir ~/.ai-memory/memory/journal
```
...
```bash
bash ~/repos/greg/ai-memory-kit/setup.sh --tool claude-code --update
```

Or install manually:
```bash
cp ~/repos/greg/ai-memory-kit/journal-summary.sh ~/.claude/memory-kit/journal-summary.sh
chmod +x ~/.claude/memory-kit/journal-summary.sh
```
